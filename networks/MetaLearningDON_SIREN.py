    ...
    def __init__(
        ...
        for layer in range(self.num_trunk_layers):
            ...
            if weight_norm and layer in self.trunk_norm_layers:
                trunk_layers.append(nn.utils.weight_norm(linear_layer))
            else:
                trunk_layers.append(linear_layer)

            if layer in self.trunk_norm_layers:
                trunk_layers.append(nn.LayerNorm(out_dim))
            if layer == 0:
                trunk_layers.append(SinActivation(omega=omega_0))
            else:
                trunk_layers.append(SinActivation(omega=1.0))
